...
function rootReducer (state = initialState, action){
    if (action.type === ADD_CART) {
        ...
        if(p.id === action.payload.id){ esta = true } } )
        if(!esta){ return { ...state, carrito: state.carrito.concat(action.payload) } }
    }
    if (action.type === REMOVE_CART){
        return{ ...state, carrito: state.carrito.filter((e) => e.id !== action.payload.id) }
    }
    ...
    if(action.type === SET_USER){
        return{ ...state, user : { id : action.payload.id, isAdmin : action.payload.isAdmin, name: action.payload.name } }
    }
